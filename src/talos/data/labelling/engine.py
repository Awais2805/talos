...
The order is not arbitrary. Rules load and validate before any data is read, so
...
from talos.data.labelling.join import IntervalJoinBuilder
from talos.data.labelling.manifest import AttackRule, ManifestLoader
...
class LabellingEngine:
    ...
    def __init__(self, cfg, lake=None, manifests: str | Path | None = None,
        ...
        self.writer = LabelWriter(self.execution, ClosedWorldResolver(), self.provenance)
    ...
    def label(self, dataset: str, feature_space: str, source: str | None = None,
        ...
        if write:
            ...
            rel = self.writer.stamp(rel, dataset, report.manifest_sha,
                                    report.taxonomy_sha, report.labelled_at)
            report.output = self.writer.write(
                self.lake, rel, dataset, feature_space,
                source=self.cfg.source_of(dataset),
                manifest_sha=report.manifest_sha, force=force)
            self.provenance.run_report(f"labelling_{dataset}", report.to_dict())
        return report
