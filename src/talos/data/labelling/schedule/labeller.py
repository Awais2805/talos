...
class ScheduleLabeller(LabellingMethod):
    ...
    def __init__(self, cfg, lake=None, spec=None, manifests=None, taxonomy=None,
                 allow_untrained: bool = False, skip_audit_gate: bool = False,
                 **kwargs):
        ...
        """
        # `allow_untrained`/`skip_audit_gate` are accepted and unused: schedule
        # labelling learns nothing and has no D_s to fine-tune on, so neither
        # flag applies -- kept as explicit params (not swept into **kwargs) so
        # they are never forwarded to LabellingEngine, which does not expect them.
