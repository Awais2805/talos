...
from talos.data.labelling.schedule.quarantine import QuarantineLoader, QuarantineRegion
...
#: The manifest plug-in point: a user brings their own attack schedule for a
#: dataset, or one for a dataset the package has never heard of. Their directory
#: is searched first, and `Declaration.origin` is what stops the substitution
#: being invisible in the run report.
MANIFESTS = ConfigPlugins("manifest", built_in=DEFAULT_DIR, error=ManifestError)
...
class ManifestLoader:
    ...
    def __init__(self, directory: str | Path | None = None):
        """Reads the shared `MANIFESTS` point, so config.yml overrides apply.

        A `directory` scopes this loader to one place instead -- for a test, or
        to read a directory deliberately without the user's search paths. Such a
        loader is private: it does not claim the `manifest` kind.
        """
        self.specs = MANIFESTS if directory is None else ConfigPlugins(
            "manifest", built_in=directory, error=ManifestError, addressable=False)
    ...
    def path_for(self, dataset: str) -> Path:
        return self.specs.path_for(dataset)

    # ------------------------------------------------------------------ load

    def load(self, dataset: str, taxonomy: TaxonomyMapper | None = None) -> Manifest:
        taxonomy = taxonomy or TaxonomyMapper()
        declaration = self.specs.get(dataset)
        path, doc = declaration.path, declaration.doc
